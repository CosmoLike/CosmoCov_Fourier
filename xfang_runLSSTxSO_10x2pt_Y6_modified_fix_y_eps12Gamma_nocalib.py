#!/usr/bin/env python
## matter part halo parameters not calibrated by tSZ, {eps1, eps2, Gamma} are fixed only for y part.
...
chain_file = os.path.join(dirname, "chains/LSSTxSO_10x2pt_model_%d_modified_fix_y_eps12Gamma_nocalib" %model)
...
sample_params = sample_cosmology_10x2_fix_y_eps12Gamma(get_N_tomo_shear(),get_N_tomo_clustering(), MG=False, w0wa=False, cov_modified=True)
