...
/* here, do your time-consuming job */
  ...
  double sigma_zphot_shear[N_scenarios]={0.05,0.05,0.05, 0.01,0.01};
  double sigma_zphot_clustering[N_scenarios]={0.03,0.03,0.03, 0.01,0.01};

  double area_table[N_scenarios]={12300.0,16500.0,18000.0, 2000.0,2000.0}; // Y1 corresponds to DESC SRD Y1, Y6 corresponds to assuming that we cover the full SO area=0.4*fsky and at a depth of 26.1 which is in a range of reasonable scenarios (see https://github.com/LSSTDESC/ObsStrat/tree/static/static )
  // double nsource_table[2]={11.0,23.0};
  // double nlens_table[2]={18.0,41.0};

  double nsource_table[N_scenarios]={10.7,22.5,27.1, 66.0,66.0};
  double nlens_table[N_scenarios]={13.1,26.8,32.0, 51.0,51.0};

  char survey_designation[N_scenarios][200]={"LSSTxSO_Y1","LSSTxSO_Y6","LSSTxSO_Y10","RomanxSO","RomanxS4"};
  char tomo_binning_source[N_scenarios][200]={"source_std","source_std","source_std","source_std","source_std"};
  ...
  char tomo_binning_lens[N_scenarios][200]={"lens=src","lens=src","lens=src","lens=src","lens=src"};
  ...
  sigma_zphot_clustering[4] = sigma_zphot_shear[4];
#else
  ...
  char tomo_binning_lens[N_scenarios][200]={"LSST_gold","LSST_gold","LSST_gold","WF_SN10","WF_SN10"};
#endif
  char cmb_yr[N_scenarios][100]={"so_Y1","so_Y5","so_Y5","so_Y5","s4"};
  ...
  if (sce == 3 || sce == 4) { init_binning_fourier(25,20.0,7979.0,4000.0,21.0,10,10); } // Roman use lmax_shear=4000
