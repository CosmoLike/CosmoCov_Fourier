...
int main(int argc, char** argv)
{
...
#else
  sprintf(covparams.outdir,"out_cov_lsstxso_3x2pt/");
  // sprintf(covparams.outdir,"tests/new2/");
#endif
#ifdef SLOW
  sprintf(covparams.outdir,"out_cov_lsstxso_3x2pt_slow/");
#endif
#ifdef RUN_FFT
  sprintf(covparams.outdir,"out_cov_lsstxso_3x2pt_fft/");
#endif

  printf("----------------------------------\n");
  char ABCD[2][4];
  const char *probes[3] = {"ss", "ls", "ll"};
  const int Npower[3] = {tomo.shear_Npowerspectra,tomo.ggl_Npowerspectra,tomo.clustering_Npowerspectra};
  int N_blocks[7], i_block=1; // There are 6 blocks in total
  int i_probe1[7], i_probe2[7];
  N_blocks[0]=0;
  ...
  sprintf(OUTFILE,"hf_%s_%s%s_cov_Ncl%d_Ntomo%d",survey.name,probes[i_pro1],probes[i_pro2], like.Ncl,tomo.shear_Nbin);
